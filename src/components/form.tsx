...
import { CopySchema } from '~/server/api/routers/file/file.schema';
...
const GoogleDriveForm = memo(() => {
  ...
  const [form, fields] = useForm({
    onValidate({ formData }) {
      return parseWithZod(formData, { schema: CopySchema });
    },
    shouldValidate: 'onBlur',
    shouldRevalidate: 'onInput',
    onSubmit: (event, context) => {
      event.preventDefault();
      const data = parseWithZod(context.formData, { schema: CopySchema });
    ...
    >
      <div className="relative w-full">
        ...
        {fields.url.errors && (
          <p
            role="alert"
            className="-bottom-2 absolute left-0 translate-y-full text-red-500 text-xs"
          >
            {fields.url.errors}
          </p>
        )}
